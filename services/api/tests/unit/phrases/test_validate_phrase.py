"""Unit tests for `ValidatePhrase` (tasks.md 3.1). Uses
`InMemoryUnitOfWorkFactory` + `FakeEmbedder`; stored phrases are placed at a
controlled cosine distance from the query vector `PROBE` via
`tests/contract_suite/vectors.py`'s `vector_at_distance`, matching the
contract-suite's own fixtures.
"""
...
def test_widened_bound_admits_a_borderline_row_that_the_tail_rule_then_excludes() -> None:
    # A: raw cosine 0.90 -> score 0.90, a clear match.
    # B: raw cosine 0.79994 -> score 0.7999, admitted by the widened SQL bound
    #    (max_distance = 0.2001 at threshold 0.80) but excluded by the exact
    #    Decimal comparison -- design.md's "rounding boundary" table.
    # C: exists only so the repository's own `+1` probe reports `has_more`
    #    True; the tail rule must override that to False once B truncates.
    factory = InMemoryUnitOfWorkFactory()
