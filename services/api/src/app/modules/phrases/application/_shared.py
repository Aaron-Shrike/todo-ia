"""Shared view types and the tail-rule reconciliation helper for the three
Unit 3 use cases (`ValidatePhrase`, `ListMatches`, `SavePhrase`). Not in
tasks.md's literal file list -- the "validate-shaped" result (design.md:
"The 409 body carries a complete validate response") is built by
`ValidatePhrase` AND by `SavePhrase`'s 409 `details`, so it is built once
...
def build_matches_page(
    ...
    """The tail rule (design.md "Match query and keyset pagination"):
    `find_matches`' widened SQL bound over-selects, so the application stops
    at the first row failing `policy.includes` -- monotonic in distance, so
    every later row would also fail -- and that truncation forces
    `has_more=False`/`next_cursor=None`, overriding the repository's own
    `+1`-probe answer."""
    kept: list[Match] = []
    truncated = False
