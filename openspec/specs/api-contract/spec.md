...
- GIVEN cursors that decode to a JSON object but violate one field rule each: wrong `v`; `t` not a string; `d` negative, above 2, NaN or a string; `i` zero, negative, above int64 or `true`; `th` below 0, above 1 or non-finite; a missing or extra key; or an encoded length above the maximum
- WHEN matches is called
